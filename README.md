...
vue作为单页面应用已经无可挑剔，但在真实的项目环境里，往往是需要对每张页面都部署一套vue体系(微博监控后台/微信监控后台/配置系统发布/etc)，
这时候部署多套vue编译系统显然并不现实，但是网上又没有现成可用的多页面vue项目可供利用，所以就有了这个项目。

搭建系统的过程中踩到了各种各样的坑，都在项目文档和代码注释中进行了详细的说明，如果还有不甚清楚的地方，欢迎开issue提问
